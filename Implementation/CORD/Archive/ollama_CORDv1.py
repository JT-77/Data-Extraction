...
# Configure logging to only show WARNING and ERROR, or disable it
# For this request, we'll use print statements for the specific output needed.
# If you want to completely disable logging, you can set level to CRITICAL or higher.
...
# --- Configuration ---
OLLAMA_HOST = "http://localhost:11434" # IMPORTANT: Set your Ollama server host here
# OLLAMA_MODEL_NAME = "mistral-small3.2:latest"
OLLAMA_MODEL_NAME = "qwen2.5vl:latest"
# OLLAMA_MODEL_NAME = "llama3.2-vision:latest"
MAX_SAMPLES_TO_PROCESS = 5 # Limit processing to this many samples for debugging

# --- Define the specific labels we care about for KIE evaluation as a frozenset ---
# These are the *potential* labels. The prompt will dynamically select from these per sample.
...
# --- Helper to convert PIL Image to base64 for Ollama (unchanged) ---
...
# --- Prompt for Ollama (Dynamically generated based on present GT labels) ---
def create_ollama_prompt_for_cord(labels_to_request: frozenset):
    ...
    if "menu.price" in labels_to_request:
        ...
        menu_item_example["price"] = "10.50" # Example: if count is 2, and unit price is 3.00
...
# --- Function to call Ollama using requests.post (unchanged) ---
...
# --- Post-process Ollama Output to Flattened Entities (Streamlined) ---
...
async def evaluate_ollama_on_cord():
    print("--- Starting Ollama evaluation for CORD v2 (KIE Metrics) ---")
    ...
    for i, example in enumerate(test_dataset):
        ...
        # print(f"\n--- Generated Dynamic Ollama Prompt for Sample {i+1} --- \n{prompt}\n-----------------------------") # Removed for minimal logging
        ...
        if ollama_output:
            print(f"Ollama output received for sample {i+1}. Post-processing...")
            ...
            print("\n--- Ollama Predicted Entities (Flattened & Normalized) (Matched Only) ---")
            ...
            print(f"Ollama call failed or returned no output for sample {i+1}. Skipping KIE evaluation for this sample.", file=sys.stderr)
            # No entities added to all_predicted_flattened_entities_list, effectively counting as FNs

    print("\nCalculating final KIE metrics for Ollama across all processed samples...")
    final_kie_metrics_results = calculate_kie_metrics(all_gt_flattened_entities_list, all_predicted_flattened_entities_list)

    print(f"\n--- Ollama CORD v2 Evaluation Results (KIE Metrics) for model {OLLAMA_MODEL_NAME} ---")
    ...
    print("\n--- Ollama CORD v2 evaluation finished! ---")
