...
# --- Prompt for Gemini (for DocVQA) ---
def create_gemini_prompt_for_docvqa(question: str):
    """
    Creates a concise prompt for Gemini to answer a question based on an image.
    """
    prompt_parts = []
        ...
        try:
            response = model.generate_content(
                ...
                generation_config=genai.GenerationConfig(
                    temperature=0.1,
                    max_output_tokens=512 # Limiting output length for concise answers
                ),
...
def evaluate_gemini_on_docvqa():
    ...
    for i, example in enumerate(test_dataset):
        ...
        if predicted_answer is not None:
            ...
            print(f"Gemini call failed or returned no output for sample {i+1}. Skipping VQA evaluation for this sample.", file=sys.stderr)

    print(f"\n--- Evaluation Summary for {GEMINI_MODEL_NAME} ---")
