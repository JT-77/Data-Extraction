...
Analyze the attached receipt image and extract its complete text to find the below information ->
    ...
    Only output JSON format in your response (nothing else, no text).
```
"""
...
def normalize_string_for_date_parsing(date_str):
    normalized = date_str.replace('-', ' ').replace('.', ' ').replace(',', ' ').replace('/', ' ').strip()
    return normalized
    ...
    for key in entity_keys:
        ...
        if llm_value != "": # If LLM extracted something non-empty (not "null" string, not empty string)
            ...
            if key in ["company", "address"]:
                # Use fuzzy ratio for company and address (threshold 80)
                if fuzz.ratio(gt_value, llm_value) >= 80: 
                    is_match = True
            elif key == "date":
                gt_value_normalized = normalize_string_for_date_parsing(gt_value)
                llm_value_normalized = normalize_string_for_date_parsing(llm_value)

                gt_date_obj = None
                llm_date_obj = None

                try:
                    gt_date_obj = dateutil_parse(gt_value_normalized, fuzzy=True, dayfirst=False)
                except ParserError:
                    pass

                try:
                    llm_date_obj = dateutil_parse(llm_value_normalized, fuzzy=True, dayfirst=False)
                except ParserError:
                    pass

                if gt_date_obj:
                    try:
                        gt_date_obj = dateutil_parse(gt_value_normalized, fuzzy=True, dayfirst=True)
                    except ParserError:
                        try: # Fallback to monthfirst if dayfirst failed
                            gt_date_obj = dateutil_parse(gt_value_normalized, fuzzy=True, dayfirst=False)
                        except ParserError:
                            gt_date_obj = None # Still unparseable

                if llm_date_obj:
                    try:
                        llm_date_obj = dateutil_parse(llm_value_normalized, fuzzy=True, dayfirst=True)
                    except ParserError:
                        try:
                            llm_date_obj = dateutil_parse(llm_value_normalized, fuzzy=True, dayfirst=False)
                        except ParserError:
                ...
                # If both can be parsed into valid datetime objects, compare them.
                if gt_date_obj and llm_date_obj:
                    is_match = (gt_date_obj.date() == llm_date_obj.date()) # Compare only date part (ignore time)
                    if not is_match:
                        print(f"  Date Mismatch: GT='{gt_value}' ({gt_date_obj.date()}), LLM='{llm_value}' ({llm_date_obj.date()})")
                else:
                    # If either cannot be parsed, it's a mismatch.
                    is_match = False
                    if not gt_date_obj:
                        print(f"  GT Date Unparseable: '{gt_value}' (Normalized: '{gt_value_normalized}')")
                    if not llm_date_obj:
                        print(f"  LLM Date Unparseable: '{llm_value}' (Normalized: '{llm_value_normalized}')")
...
async def evaluate_sroie(num_samples=None):
    ...
    for i, doc_id in enumerate(all_doc_ids):
        ...
        # print(f"DEBUG: Looking for image at: {os.path.join(SROIE_TEST_DATA_PATH, 'img', f'{doc_id}.jpg')}")
        # print(f"DEBUG: Looking for JSON at: {os.path.join(SROIE_TEST_DATA_PATH, 'entities', f'{doc_id}.txt')}")
        ...
        print(f"LLM output - {llm_output_json}")  # DEBUG: Print LLM output for inspection
